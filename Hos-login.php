...
include 'connect.php';
 session_start();
  if(isset($_REQUEST['submit'])){
    ...
    $email = $_POST['Email'];
    $pass = $_POST['Password'];
    $sql = "Select  * from Hospital_register  where  Email = '$email' and Password = '$pass'";
    $result= mysqli_query($conn,$sql);
    if(mysqli_num_rows($result)>0)
    {
     $_SESSION['Email'] = $email;
     header('location:Hospital-home-page.php');
 
    }else{
        header('location:Hos-login.php');
    }
 }
 if(isset($_REQUEST['login-out'])){
  session_unset();
  session_destroy();
}
?>
